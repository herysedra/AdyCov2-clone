...
@with_kw mutable struct CoVParameters_AS
    ...
    #MS
    MS_strategy::Vector{Float64} = zeros(370)
    MS_probaₐ=zeros(n_a)    #testing probability per age group
    MS_nb_testsₐ=zeros(n_a) #number of tests per age group
    MS_probaₐₛ=zeros(n_a,n_s) #testing probabiliy per age and per state
end
...
#=@with_kw mutable struct CoVParameters_AS
    ...
    #MS
    MS_strategy::Vector{Float64} = zeros(370)
    #testing_uₚ::Array{Array{Float64,2},1} = [zeros(n_a,n_s)    for wa=1:n_wa] # for inplace calculations (probability array)
    #testing_nbTests::Array{Array{Float64,2},1} = [zeros(n_a,n_s)    for wa=1:n_wa]  # for inplace calculations, at t, how many tests to apply in each wa, a, and s
    MS_probaₐ=zeros(n_a)    #testing probability per age group
    MS_nb_testsₐ=zeros(n_a) #number of tests per age group
    MS_probaₐₛ=zeros(n_a,n_s) #testing probabiliy per age and per state
    #MS_nb_testsₐₛ=zeros(n_a,n_s) #number of tests per age and per state
    #test_indices_per_age=zeros(Int(floor(maximum(testing_strategy))))
end
=#
