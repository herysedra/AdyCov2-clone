...
u0,P,P_dest = KenyaCoV.model_ingredients_from_data("data/data_for_age_structuredmodel.jld2",
    ...
    P.β = 2.5#rand(KenyaCoV.d_R₀) #Choose R₀ randomly from 2-3 range

    u0[4,8,3] = 5 #10 initial Asymptomatics in Nairobi
...
prob = KenyaCoV.create_KenyaCoV_non_neg_prob(u0,(0.,1*365.),P)
    ...
    display(plot([sum(sims_baseline.u[1][t][9:11])     for t = 1:1:365]))
    println(sum(sims_baseline.u[1][365][9:11]))

#Testing with the ODE version
prob_ode = KenyaCoV.create_KenyaCoV_ode_prob(u0,(0.,365.),P)
    @time sol_ode = solve(prob_ode,Tsit5())
    display(plot([sum(sol_ode.u[t][:,:,9:11])     for t = 1:1:size(sol_ode.u,1)]))
    println("5 nairobi -> ",sum(sol_ode.u[end][:,:,9:11]))

    #V = H, just H is a bit delayed -> OK
    #plot([sum(sol_ode.u[t][:,:,11])     for t = 1:1:size(sol_ode.u,1)])
    #display(plot!([sum(sol_ode.u[t][:,:,12])     for t = 1:1:size(sol_ode.u,1)]))
