...
"""
u0,P_AS,P_dest = KenyaCoV.model_ingredients_from_data(joinpath(dirname(pathof(KenyaCoV)),"../data/data_for_age_structuredmodel.jld2"),
                                                    joinpath(dirname(pathof(KenyaCoV)),"../data/flight_numbers.csv"),
                                                    joinpath(dirname(pathof(KenyaCoV)),"../data/projected_global_prevelance.csv"))

"""
Convert the basic spatial and age-structure model into a model with explicit moves by
EITHER
Giving information about the agestructed model, matrix of destination probabilities, the return rate for each area, and the leaving rates
OR
Giving information about the agestructed model, matrix of destination probabilities, and the return rate for each area
"""
#Leaving and returning rates
# P = KenyaCoV.generate_explicitmove_parameters(P_AS,(1/5)*ones(20),[0.01 for i = 1:20],P_dest)
P = KenyaCoV.generate_explicitmove_parameters(P_AS,(1/5)*ones(20),P_dest)
