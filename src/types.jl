...
@with_kw mutable struct CoVParameters <: AbstractCovParameters
    β::Float64 = 2.5/3.6
    γ::Float64 = 1/3.6
    σ::Float64 = 1/2.
...
@with_kw mutable struct CoVParameters_AS <: AbstractCovParameters
    ...
    μ₁::Float64 = 0.01#Excess mortality due to disease
    ϵ::Float64 = 0.1 #Relative infectiousness of undetectable infecteds
    χ::Vector{Float64} = ones(n_a) #Age-dependent relative susceptibilities
    ρ::Vector{Float64} = [0.01 for i in 1:n_wa] #Time spent outside of area
    T::Matrix{Float64} = zeros(n_wa,n_wa)#Probability distributions of location for mobile individuals
...
@with_kw mutable struct CoVParameters_AS_EM <: AbstractCovParameters
    β::Float64 = 2.5/3.6
    γ::Float64 = 1/3.6
    σ::Float64 = 1/2.
    δ::Float64 = 0.05#Proportion of symptomatic/diseased vs non-symptomatic cases
    ...
    χ::Vector{Float64} = ones(n_a) #Age-dependent relative susceptibilities
    ρ::Vector{Float64} = [0.01 for i in 1:n_wa] #Time spent outside of area
    r::Vector{Float64} = [1/5 for i = 1:n_wa] #Returning rate
    l::Vector{Float64} = [1. for i = 1:n_wa] #Leaving rate
    ...
    N̂::Matrix{Float64} = zeros(n_wa,n_a)#For inplace calculations
    λ::Matrix{Float64} = zeros(n_wa,n_a)#For inplace calculations
    dN::Vector{Int64} = zeros(Int64,n_wa*n_wa*n_a*n_ta)#For inplace calculations
    poi_rates::Vector{Float64} = zeros(n_wa*n_wa*n_a*n_ta)#For inplace calculations
    dc::SparseMatrixCSC{Int64,Int64} = sparse(zeros(Int64,n_wa*n_wa*n_a*n_s,n_wa*n_a*n_ta))#For inplace calculations
    du_linear::Vector{Int64} = zeros(Int64,n_wa*n_wa*n_a*n_s) #for inplace calculations
    du_moves::Array{Int64,4} = zeros(Int64,n_wa,n_wa,n_a,n_s) #for inplace calculations
end
